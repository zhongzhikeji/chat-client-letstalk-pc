<template>
      ...
      <div style="height: 8%">
        <van-nav-bar>
          <template #left>
            ...
            <span style="margin-left: 10px">移除群管理员</span>
          </template>
        </van-nav-bar>
      </div>
  ...
  import { findMyFriends, addgroup ,findGroupMember,setgroupGuilder,removegroupGuilder} from "@/request/http.api.ts";
  ...
  const changeAdd = async () => {
    ...
    removegroupGuilder(obj).then(res => {
      console.log(res);
