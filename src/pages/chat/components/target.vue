        ...
        <div class="center">
            ...
            <div class="group">
                ...
                <van-collapse v-model="activeNames">
                    <!-- <van-collapse-item title="联络人" name="1">
                        <div class="list" v-for="(item, index) in userList" :key="index">
                            <div
                                ...
                                <span style="margin-left: 24px">{{ item.displayName }}</span>
                            </div>
                    ...
                    <van-collapse-item title="群组" name="2">
                        <div class="list" v-for="(item, index) in groupList" :key="index">
                            <div
                                style="display: flex; align-items: center; margin-left: 10px border-bottom: 1px solid #000;">
                                <van-image width="36" height="36"
                                    :src="item.groupHead" round />
...
import { findMyFriends, findGroup, getBoID, botInfo } from "@/request/http.api.ts";
...
const toggle = computed(async () => {
    ...
    let targetIdsToUser = []
    let targetIdsToGroup = []
    pushList.value.forEach(item => {
        console.log(item);
        if (item.id) {
            targetIdsToGroup.push(item.id)
        } else {
            targetIdsToGroup.push(item.groupId)
        }
    })
    obj = {
        targetIdsToUser,
...
const getGroup = async () => {
    ...
    res.data.forEach((item, index) => {
        if (item.groupId == localStorage.getItem("groupId")) {
